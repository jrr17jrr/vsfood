...
import { getPainelOrders, getRestaurant } from "@/lib/data/painel";
...
export default async function PainelPedidosPage() {
  ...
  const [restaurant, orders] = await Promise.all([getRestaurant(restaurantId), getPainelOrders(restaurantId)]);
  ...
  return (
    <div>
      ...
      <div className="mt-6">
        <OrdersBoard orders={orders} restaurant={restaurant} />
      </div>
    </div>
  );
}
