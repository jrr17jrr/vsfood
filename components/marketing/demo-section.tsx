...
import { ArrowRight, BookOpenText, Palette, ShoppingBag, Smartphone } from "lucide-react";
...
export async function DemoSection() {
  ...
  return (
    <section id="demonstracao" className="relative overflow-hidden py-20 sm:py-28">
      <div className="mx-auto max-w-6xl px-4">
        <div className="grid items-center gap-10 lg:grid-cols-2">
          <Reveal>
            <h2 className="text-3xl font-bold tracking-tight sm:text-4xl">
              Veja o VSFood funcionando de verdade
            </h2>
          ...
          <Reveal direction="right" delayMs={100}>
            <Link
              href={`/loja/${demo.slug}`}
              className="group relative block rounded-3xl transition-transform duration-300 hover:-translate-y-1"
            >
              <div
                className="absolute -inset-4 -z-10 rounded-[2.5rem] bg-gradient-to-br from-brand-orange/20 to-brand-red/10 opacity-0 blur-2xl transition-opacity duration-300 group-hover:opacity-100"
                aria-hidden
              />
              <div className="overflow-hidden rounded-3xl border border-border/70 bg-card shadow-xl shadow-black/10 dark:shadow-black/40">
                <div className="relative aspect-[16/7] w-full bg-muted sm:aspect-[2/1]">
                  {demo.banner_url ? (
                    <Image
                      src={demo.banner_url}
                      alt=""
                ...
                <div className="flex items-center gap-3 p-5">
                  <div className="relative size-14 shrink-0 overflow-hidden rounded-2xl border border-border/60 bg-white p-1.5 shadow-sm">
                    {demo.logo_url ? (
